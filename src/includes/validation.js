...
import {
  required, min, max, alpha_spaces as alphaSpaces, email, max_value as maxValue,
  min_value as minValue, confirmed, is_not as excluded,
  ...
  install(app) {
    ...
    configure({
      generateMessage: (ctx) => {
        ...
        return messages[ctx.rule.name] ? messages[ctx.rule.name] : `The field ${ctx.field} is not valid`;
      },
    });
  },
};
